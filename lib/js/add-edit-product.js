(function(it_exchange_variants_add_edit_product) {
	...
	it_exchange_variants_add_edit_product(window.jQuery, window, document);
		...
		$(function() {
			...
			$('a.it-exchange-variant-preset-template-title').on('click', function(event) {
				...
				$.post(ajaxurl, data, function(response) {
					...
					if ( response.status ) {
						...
						$( '.it-exchange-existing-variants' ).removeClass( 'no-variants' );
			...
			// When removing variants, let's check if we need to add the no-variants class.
			$( '.it-exchange-existing-variants' ).on( 'click', '.it-exchange-remove-item', function() {
				if ( $( '.it-exchange-existing-variant' ).length == 0 ) {
					$( '.it-exchange-existing-variants' ).addClass( 'no-variants' );
				}
			});
