		...
		$(function() {
			...
			add_images_frame = {
				...
				select: function() {
					...
					hiddenField = '<input type="hidden" name="varient-value-image[]" value="' + source.sizes.full.url + '" />';

					$( '.variant-values-list' ).find( 'li[data-variant-value-id="' + variant_value_id + '"] .variant-value-image' ).html( image );
					$( '.variant-values-list' ).find( 'li[data-variant-value-id="' + variant_value_id + '"]').append( hiddenField );
				},
