...
(function($){
	...
	*/
	itExchangeVariants.AdminMetaBoxView = Backbone.View.extend({
		...
		// Show update notification and toggle hidden field to trigger save when variant data is changed
		variantsDataUpdated: function() {
			$('.variants-updated-notification')
				.fadeIn()
				.find(':hidden').val(true);
		},
		...
		*/
		...
		makeVariantsSortable: function() {
			...
			$( '.it-exchange-existing-variants' ).sortable({
				...
				start: function( e, ui ) {
					itExchangeVariants.checkedDefaultValues = [];
					$('.variant-radio-option', $(ui.item)).each(function(){
						if ( $(this).is(':checked') ) {
							itExchangeVariants.checkedDefaultValues[$(this).val()] = true;
						}
					});
				...
				stop: function( e, ui ) {
					...
					$('.variant-radio-option', $(ui.item)).each(function(){
						if ( 'undefined' != typeof itExchangeVariants.checkedDefaultValues[$(this).val()]) {
		...
		*/
		...
		addNewVariantToCollectionFromTemplate: function(id){
			if ( this.corePresets.get(id)) {
				...
				var newVariant = {
					'title'        : '',
					'placeholder'  : presetVariant.get('placeholder'),
		...
		addNewVariantToCollectionFromSaved: function(id){
			if ( this.savedPresets.get(id)) {
				...
				var newVariant = {
					...
					'placeholder'  : savedVariant.get('placeholder'),
	...
	itExchangeVariants.ProductVariantView = Backbone.View.extend({
		...
		render : function () {
			...
			if ( openDiv ) {
				...
				$('.variant-text-placeholder', this.$el).trigger('click');
			}
			return this;
		},
		...
		events: {
			...
			'click .add-variant-value-button': 'addNewVariantValue'
		},
		...
		makeVariantValuesSortable: function() {
			...
			$('.variant-values-list', this.$el ).sortable({
				...
				start: function( e, ui ) {
					...
					$( this ).addClass( 'sorting' );
					itExchangeVariants.checkedDefaultValues = [];
					$('.variant-radio-option', $(this)).each(function(){
						if ( $(this).is(':checked') ) {
							itExchangeVariants.checkedDefaultValues[$(this).val()] = true;
						}
					});
		...
		makeVariantTitlesEditable: function(event) {
			...
			$parent.find('.variant-text-input')
			 ...
			 // Give a little delay and foucs
			 setTimeout(function(){
				 $parent.find('.variant-text-input').focus();
			 }, 1);
		},
		...
		deleteVariant: function(event) {
			event.stopPropagation();
			event.preventDefault();
			if ( confirm( itExchangeVariantsI18n.confirmDeleteVariant ) ) {
		...
		addOneVariantValue: function ( variantValue ) {
			...
			// Reindex all variant order when adding a new variant
			$('.variant-order-input', this.$variantValues).each(function(index){
				$(this).val(index);
			});
		},

		// Add a new variant Value for an existing Variant
		addNewVariantValue: function() {
			// Init new value
			var variantValue = new itExchangeVariants.VariantValue();
			var that         = this;

			// Set URL to pick up new value from the type of UI it is (select, radio, color, image)
			variantValue.url = ajaxurl + '?action=it-exchange-variants-json-api&endpoint=variant-value-from-ui-type&ui-type=' + this.model.get('uiType') + '&parent-id=' + this.model.get('id');

			// Fetch the new vwale and add to the models on success
			variantValue.fetch({
				variant: this.model,
				success: function( model, response, options ) {
					options.variant.values.add(model);

					// Make sure we have a default value checked
					if (!$('.variant-values-list', that.$el ).find('.variant-radio-option:checked').length){
						$('.variant-values-list', that.$el).find('.variant-radio-option:last').prop('checked',true);
					}

					// Focus on last value (the one we just added)
					$('.variant-values-list li:last', that.$el ).find('.variant-text-placeholder').trigger('click');
				}
			});
		}
	...
	itExchangeVariants.VariantValueView = Backbone.View.extend({
		...
		render : function () {
			...
			this.$el.html( this.template( this.model.toJSON() ) );
			$(this.$el)
				.data('variant-value-id', this.model.get('id'))
				.attr('data-variant-value-id', this.model.get('id'))
				.data('variant-value-parent', this.model.get('parentId'))
				.attr('data-variant-value-parent', this.model.get('parentId'));
			return this;
		},
		...
		// Delete variant value row when x is clicked
		deleteVariantValue: function(event) {
			event.stopPropagation();
			event.preventDefault();
			this.$elParent = this.$el.closest('.variant-values-list');
			this.$el.fadeOut().remove();
			itExchangeVariants.adminMetaBoxView.productVariants.get(this.model.get('parentId')).values.remove(this.model);

			// Make sure we still have a default value checked
			if (!this.$elParent.find('.variant-radio-option:checked').length){
				this.$elParent.find('.variant-radio-option:last').prop('checked',true);
			}
		}
