	...
	*/
	itExchangeVariants.AdminMetaBoxView = Backbone.View.extend({
		...
		*/
		events : {
			...
			'mouseleave .it-exchange-new-variant-presets': 'hideVariantPresetsPopup',
		...
		*/
		addOneVariant: function ( variant ) {
			...
			if ( variant.get('newFromPreset') ) {
				variant.values.url = ajaxurl + '?action=it-exchange-variants-json-api&endpoint=variant-values-from-preset&preset-id=' + variant.get('newFromPreset');
			}
		...
		*/
		...
		/**
		 * Hide the variant preset popup
		*/
		hideVariantPresetsPopup: function() {
		...
		*/
		...
		addNewVariantToCollectionFromTemplate: function(id){
			if ( this.corePresets.get(id)) {
				...
				newVariant.set('id','newVariant' + newVariant.cid);
			}
		},
	...
	itExchangeVariants.ProductVariantView = Backbone.View.extend({
		...
		events: {
			...
			'click .it-exchange-remove-item':  'deleteVariant'
		},
