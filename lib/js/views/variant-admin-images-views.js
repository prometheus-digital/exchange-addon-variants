...
(function($){
	...
	*/
	itExchangeVariants.ProductImagesVariantsMetaBoxView = Backbone.View.extend({
		...
		*/
		events : {
			'click #it-exchange-variant-images-add-combo-button': 'showNewImageComboSelect',
			'click #it-exchange-cancel-variant-images-create-combo' : 'hideNewImageComboSelect',
		...
		*/
		...
		/**
		 * Adds a new variant combination box
		*/
		showNewImageComboSelect : function(event) {
			// Prevent link click from refreshing page
			event.preventDefault();

			// Hide the button
			$('#it-exchange-variant-images-add-combo-button').hide();

			// Show the combo select
			$('.it-exchange-select-new-variant-images-combo-div').slideDown();
		},

		/**
		 * Adds a new variant combination box
		*/
		hideNewImageComboSelect : function(event) {
			// Prevent link click from refreshing page
			event.preventDefault();

			// Hide the combo select
			$('.it-exchange-select-new-variant-images-combo-div').hide();
				// Show the button
			$('#it-exchange-variant-images-add-combo-button').show();
		},
		...
		addNewImageComboDiv : function(event) {
			...
			if ( selectedVariantsforCombo.length > 0 ) {
				...
				$.get( ajaxurl + '?action=it-exchange-variants-json-api&endpoint=get-atts-from-raw-combo&variants-array[]=' + selectedVariantsforCombo, function(result) {
					...
					that.hideNewImageComboSelect(event);
