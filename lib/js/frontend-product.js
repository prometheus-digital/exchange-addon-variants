...
jQuery(document).ready(function($) {
	...
	*/
	$('.it-exchange-product-variants').on('click', '.it-variant-color', function(event) {
		...
		var variantValueTitle = $(this).find('.it-variant-color-inner').attr('title');
		$(this)
			...
			.closest('.it-variant-options')
				.find('.it-exchange-selected-variants-field')
					...
					.trigger('change')
					.end()
				.find('.it-variant-title-selected')
					.text(variantValueTitle);
	});
	...
	*/
	$('.it-exchange-product-variants').on('click', '.it-variant-image', function(event) {
		var variantValue      = $(this).find('img, div').attr('data-id');
		var variantValueTitle = $(this).find('img, div').attr('title');
		$(this)
	...
	*/
	...
	// Rebuild the buy now box when the cart is cleared
	itExchange.hooks.addAction( 'itExchangeSWEmptyCart', function() {
		$('.it-exchange-selected-variants-field:first').trigger('change');
	});

	// Reapply the combo hash to the purchase options after add-to-cart happens
						...
						if ( !itExchange.variantCombosUpdated ) {
							...
							$(result.inventory.selector).fadeOut(400, function(){
								...
								$(this).fadeIn(400, function(){
									itExchangeVariantsAddonAppendComboHashToPurchaseOptions(result.comboHash);
								});
							});
						}
					} else {
	...
	function itExchangeVariantsAddonAppendComboHashToPurchaseOptions(comboHash) {
		...
		if ( $swPurchaseOptions.length ) {
			if ( $('.it-exchange-variant-combo', $swPurchaseOptions).length ) {
				$('.it-exchange-variant-combo', $swPurchaseOptions ).val(comboHash);
			} else {
				$swPurchaseOptions.each(function(){
					$(this).append('<input type="hidden" class="it-exchange-variant-combo" name="it-exchange-combo-hash" value="' + comboHash + '" />');
				});
			}
		}
	}
